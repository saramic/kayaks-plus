describe("cart and checkout", () => {
  ...
  it("adds 3 items to cart and checks out", () => {
    ...
    cy.get("[data-testid=no-cart-message]").should(
      "contain.text",
      "No cart items, general enquiry only"
    );
    ...
    cy.get("table tfoot tr:nth-child(1) td:nth-child(1)").should(
      "have.text",
      "Total"
    );
    cy.get("table tfoot tr:nth-child(1) td:nth-child(3)").should(
      "have.text",
      "$600.60"
    );

    cy.log("When the user deletes the second item");
    cy.get("table tbody tr:nth-child(1) td:nth-child(4)").click();

    cy.log("Then the cart shows item 2 is not there");
    ...
    cy.log("When the user increments the first item");
    cy.get(
      "table tbody tr:nth-child(1) [data-testid=quantity-increment]"
    ).click();
    cy.get(
      "table tbody tr:nth-child(1) [data-testid=quantity-increment]"
    ).click();

    cy.log("Then the quantity of item 1 is 3");
    cy.get("table tbody tr:nth-child(1) td:nth-child(2)").should(
      "have.text",
      "3"
    );

    cy.log("And the cost of item 1 is $600.60");
    cy.get("table tbody tr:nth-child(1) td:nth-child(3)").should(
      "have.text",
      "$600.60"
    );

    cy.log("And the total is $900.90");
    cy.get("table tfoot tr:nth-child(1) td:nth-child(3)").should(
      "have.text",
      "$900.90"
    );

    cy.log("And the cart item count is 4");
    cy.get("[data-testid=cart-count]").should("have.text", "4");

    // TODO: doulbe decrement in tests?
    // cy.log("When the user decrements the first item");
    // cy.pause();
    // cy.get(
    //   "table tbody tr:nth-child(1) [data-testid=quantity-decrement]"
    // ).click({ force: true });
    ...
    // cy.log("And the cost of item 1 is $400.40");
    // cy.get("table tbody tr:nth-child(1) td:nth-child(3)").should(
    //   "have.text",
    //   "$400.40"
    // );

    // cy.log("And the total is $700.70");
    // cy.get("table tfoot tr:nth-child(1) td:nth-child(3)").should(
    //   "have.text",
    //   "$700.70"
    // );
